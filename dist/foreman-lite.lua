-- ============================================================================
-- FICSIT Foreman — LITE EEPROM  v0.9.1   (GENERATED by tools/bundle.py)
...
if (t.count or 0) > 0 then return math.ceil(o.count * termAll / t.count) end
return o.count
end
...
self.router = router
self.getProxy = getProxy or function(id) return component.proxy(id) end
self.epochSeconds = epochSeconds or 2
...
if not self:_hubViable(ing.name, hub) then return rollback() end
...
self.assign = {}
self._reach, self._rnb, self._failed = {}, {}, {}
local rem = {}
...
st.drain = st.drain + 1
if st.drain >= DRAIN_TIMEOUT then
pcall(function() opt.ctor:setRecipe(opt.recipe) end)
st.state, st.recipeName, st.item, st.drain, st.drainFor = "RUNNING", wantName, item, 0, nil
return true
end
...
local rNum, rDen = ing.amount, opt.out
if self.sources[ing.name] then
...
if self.sources[ing.name] then
elseif self.bufferOf[ing.name] then
self.hubDraw[ing.name] = (self.hubDraw[ing.name] or 0) + qty
else
self:tallyDraws(ing.name, qty, depth + 1, seen)
end
end
seen[item] = nil
end
...
self.router:order(item, need, bufferId, producer)
local terminal = self.router.orders[#self.router.orders]
for i = startN + 1, #self.router.orders - 1 do self.router.orders[i].term = terminal end
