-- ============================================================================
-- FICSIT Foreman — LITE EEPROM  v0.13.7   (GENERATED by tools/bundle.py)
...
local fullMs = opts.fullScanMs or 15000
...
local w0 = now()
local g = 0
while event.pull(0) ~= nil do g = g + 1; if g > 2000 then break end end
routed = routed + g
...
local r0 = now(); refresh(); refreshMs = now() - r0; lastMs = now()
elseif t - lastFull >= fullMs then
ctx.router:pump(); lastFull = t
elseif next(modules.Router._retry or {}) ~= nil then
computer.skip()
else
if event.pull(replan) == nil then refresh(); refreshMs = 0; lastMs = now() end
end
end
return ctx.router, ctx.planner
end
